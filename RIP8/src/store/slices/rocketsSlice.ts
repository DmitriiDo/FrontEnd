...
import {E_RocketStatus, T_Rocket, T_RocketsFilters, T_Component} from "modules/types.ts";
...
export const fetchRockets = createAsyncThunk<T_Rocket[], object, AsyncThunkConfig>(
    ...
    async function(_, thunkAPI) {
        ...
        return response.data.filter(rocket => rocket.owner.includes(state.rockets.filters.owner))
    }
)
...
export const acceptRocket = createAsyncThunk<void, string, AsyncThunkConfig>(
    "rockets/accept_rocket",
    async function(rocket_id,{dispatch}) {
        await api.rockets.rocketsUpdateStatusAdminUpdate(rocket_id, {status: E_RocketStatus.Completed})
        await dispatch(fetchRockets)
    }
)

export const rejectRocket = createAsyncThunk<void, string, AsyncThunkConfig>(
    "rockets/accept_rocket",
    async function(rocket_id,{dispatch}) {
        await api.rockets.rocketsUpdateStatusAdminUpdate(rocket_id, {status: E_RocketStatus.Rejected})
        await dispatch(fetchRockets)
    }
)
